...
document.getElementById('formElement12').style.visibility="hidden";//address3
document.getElementById('formElement12').style.maxHeight="0px";
document.getElementById('formElement12').style.padding="0px";
...
label.appendChild(document.createTextNode(' Sim, concordo também em receber da Avaya emails promocionais.'));
var divGDPRtext = document.createElement("div");
divGDPRtext.id="gdprtext";
divGDPRtext.innerHTML = "<div style='color: #000;width: 90%;margin-left: 25px;margin-bottom: 10px;margin-top: 15px;font-size:13px;line-height:20px;'>Ao submeter este formulário, estou de acordo que os meus dados sejam processados na base de dados da Avaya para o fim acima mencionado. Consulte a nossa <a href='https://www.avaya.com/br/privacy/website/' target='_blank' style='color:#DA291C;text-decoration:underline;font-size: 13px;border-bottom: 0px solid;'>Declaração de Privacidade</a> para mais detalhes, ou configure em qualquer momento as suas <a href='https://news.avaya.com/global-subscription' target='_blank' style='color:#DA291C;text-decoration:underline;font-size: 13px;border-bottom: 0px solid;'>preferências de email</a>.</div>";
...
/*vUsingUCSolutionYes.onclick = function(){
		...
		fe88115.enable();
		fe88115.add( Validate.Presence, { failureMessage: "Campo obrigatório" } );	
...
document.getElementById("fe110683").type = "hidden"; //EmailOptin is text field to hide this field
...
if (document.getElementById('formElement8')){
	...
	if (document.getElementById('fe110663')){
		var div71a = document.getElementById('fe110663');
...
function detect_ccode(){
		var strUser=document.getElementsByName("country")[0].value;
				...
				/*if(strUser=="United States"){
					...
					document.getElementById('formElement7').style.display = "block";
					var winWidth2 = window.outerWidth;
					if(winWidth2 > 768) {
									resizefield();
					} else {
						resizefield2();
					}
					var fe110662 = new LiveValidation("fe110662", {validMessage: "", onlyOnBlur: true});
					fe110662.enable();
					fe110662.add(Validate.Presence, {failureMessage:"Campo obrigatório"});
				} else {
					...
					var fe110662 = new LiveValidation('fe110662');
					fe110662.disable();
				}*/
...
document.getElementsByName("content_url")[0].value = "";
}
...
fe110655.add(Validate.Presence, {failureMessage:"Campo obrigatório"});
fe110656.add(Validate.Presence, {failureMessage:"Campo obrigatório"});
fe110657.add(Validate.Presence, {failureMessage:"Campo obrigatório"});
fe110657.add(Validate.Format, {pattern: /^([^@\s]+)@((?:[-a-z0-9]+\.)+[a-z]{2,})$/i, failureMessage: "Endereço de e-mail válido"});
fe110658.add(Validate.Presence, {failureMessage:"Campo obrigatório"});
fe110659.add(Validate.Presence, {failureMessage:"Campo obrigatório"});
fe110660.add(Validate.Presence, {failureMessage:"Campo obrigatório"});
fe110661.add(Validate.Presence, {failureMessage:"Campo obrigatório"});
...
var div14a = document.getElementById('fe110692');
...
function OnLoadCreateFields(){
	...
	UsingUCSolutionValid.add( Validate.Presence, { failureMessage: "Campo obrigatório" } );
...
subonclick= document.getElementById("fe110692");
...
document.getElementsByName("content_title")[0].value = document.getElementsByTagName("title")[0].innerHTML;
document.getElementsByName("lastReferralPageURL")[0].value = document.referrer;

console.log("content_title  "+document.getElementsByName("content_title")[0].value);
console.log("lastReferralPageURL1  "+document.getElementsByName("lastReferralPageURL")[0].value);
...
var imgsrc = document.getElementsByClassName('container-fluid')[0].getElementsByClassName('row');
		console.log("count  ==  "+imgsrc.length - 18);
...
for(var i = 0; i < imgsrc.length - 18; i ++) {
	...
	if (imgsrc2.length > 0){
		if (isOdd(i)){
			...
			console.log("even == "+i);
			//if(document.getElementsByName("country")[0].value=="United States" && i > 18){										   
			imgsrc[i].style.float = "left"; 
			//} else {
			//	imgsrc[i].style.float = "right"; 
			//}
			imgsrc[i].style.clear = "left"; 
			imgsrc[i].style.width = "48%"; 
			imgsrc[i].style.minHeight = "105px";
			}
		}
...
/*var choutcon1 = document.getElementsByClassName('container-fluid')[0];
var choutcon2 = choutcon1.getElementsByClassName('row');
console.log("choutcon2 === "+choutcon2.length);
for(var k = 0; k < choutcon2.length; k ++) {
	if (k == 21){
		console.log("id == "+k);
		choutcon2[k].style.display = "none";
	}
	if (k == 22){
		console.log("id == "+k);
		choutcon2[k].style.float = "right"; 
	}
}*/
...
/*GA*/
 ...
 document.getElementById('form3676').addEventListener(
