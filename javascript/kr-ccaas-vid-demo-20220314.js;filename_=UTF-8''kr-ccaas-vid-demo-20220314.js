...
/*if (!!getCookie('FName')){
	...
	labelatt('fe121764');//hide EmailOptin label
	document.getElementById("fe121764").type = "hidden";
} else {
...
if (!!getCookie('LName')){
	...
	labelatt('fe121765');//hide EmailOptin label
	document.getElementById("fe121765").type = "hidden";
} else {
...
if (!!getCookie('country')){
	...
	labelatt('fe121769');//hide EmailOptin label
	document.getElementById("fe121769").type = "hidden";
} else {
...
labelatt('fe121771');//hide EmailOptin label
document.getElementById("fe121771").type = "hidden"; //EmailOptin is text field to hide this field
...
divGDPRtext.innerHTML = "<div style='color: #000;width: 90%;margin-left: 25px;margin-bottom: 30px;margin-top: 15px;font-size:13px;line-height:20px;'>이 양식을 제출함으로써 본인은 위에서 언급한 목적으로 어바이어 데이터베이스에서 정보를 처리하는 데 동의합니다. <br>자세한 내용은 <a href='https://www.avaya.com/kr/privacy/website/' target='_blank' style='color:#DA291C;text-decoration:underline;font-size: 13px;border-bottom: 0px solid;'>개인 정보 보호 정책</a>을 참조하거나 언제든지 <a href='https://news.avaya.com/global-subscription' target='_blank' style='color:#DA291C;text-decoration:underline;font-size: 13px;border-bottom: 0px solid;'>이메일 환경 설정</a>을 관리하십시오.</div>";
...
Countrysel.value = "Korea Republic of";
...
fe121764.add(Validate.Presence, {failureMessage:"이 필드는 필수 항목입니다."});
fe121765.add(Validate.Presence, {failureMessage:"이 필드는 필수 항목입니다."});
fe121766.add(Validate.Presence, {failureMessage:"이 필드는 필수 항목입니다."});
fe121766.add(Validate.Format, {pattern: /^([^@\s]+)@((?:[-a-z0-9]+\.)+[a-z]{2,})$/i, failureMessage: "유효한 이메일 주소가 요구됩니다"});
fe121769.add(Validate.Presence, {failureMessage:"이 필드는 필수 항목입니다."});


/*Kickfire code open*/

var form = document.getElementById("form3846");
		    ...
		    if (window.jQuery) {
		        ...
		        $(function() {
		            ...
		            if( emailValue != "") {
		                ...
		                if(emailDomain != 'avaya1.com' && address3 == "") {
		                    if(emailDomain != 'sw-at1.com' && address3 == "") {
...
//GA Script
document.getElementById('form3846').addEventListener(
