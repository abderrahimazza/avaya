...
/*** GDPR Optin / Optout Code **************/
			...
			function checkOptin(){
				...
				if(checkbox1.checked == true){
					...
					document.getElementsByName("emailOptinSource1")[0].value = "WEB-GET-DE-SMB-CC";
...
vUsingCCSolutionYes.onclick = function(){
	...
	//if (document.getElementsByName('UsingCCSolution')[0].value == "Yes"){
		...
		var fe110823 = new LiveValidation( "fe110823", { validMessage: "", onlyOnBlur: true });
		fe110823.enable();
		fe110823.add( Validate.Presence, { failureMessage: "Mussfeld" } );	
	//} else {		
...
function labelatt(x) {
    ...
    for(var u = 0; u < labels.length; u ++) {
        var attr = labels[u].getAttribute('for'); //or labels[i].htmlFor
        if(attr === x) {
           labels[u].style.display = 'none';
        }
	}
}

labelatt('fe110821');
document.getElementById("fe110821").type = "hidden"; //EmailOptin is text field to hide this field
labelatt('fe110822');
document.getElementById("fe110822").type = "hidden"; 
labelatt('fe110842');
document.getElementById("fe110842").type = "hidden"; 

function labelattr(y) {
    var labels2 = document.getElementsByTagName('label');
    for(var j = 0; j < labels2.length; j ++) {
        var attr2 = labels2[j].getAttribute('for'); //or labels[i].htmlFor
        if(attr2 === y) {
           labels2[j].innerHTML = "Wenn Sie mit einem Avaya Partner oder Vertriebsmitarbeiter zusammenarbeiten, geben Sie uns bitte dessen Namen an.";
        }
	}
}
labelattr('fe110824');
...
/*if (document.getElementById('formElement7')){
	...
	if (document.getElementById('fe110821')){
		var div71a = document.getElementById('fe110821');
		var div71b = div71a.getElementsByTagName('div');
		console.log("div71b  === "+div71b.length);
		div71b[0].setAttribute("style", "width: 80px !important;display: table-cell;vertical-align: top; padding-top:10px;");
		div71b[1].setAttribute("style", "width: 80px !important;display: table-cell;vertical-align: top; padding-top:10px;");
...
if (document.getElementById('formElement7')){
	...
	var checkboxfields = document.getElementById("fe110821").innerHTML;
	document.getElementById("formElement7").innerHTML = "";
	var checkboxfieldset = "<fieldset style='height:93px;'><legend style='display: block;font-weight: bold; color: #000; width: 100%; margin-bottom: 10px; font-size:14px;'>"+fielsetTitle+"</legend>";
	checkboxfieldset = checkboxfieldset+checkboxfields+"</fieldset>";
	document.getElementById("formElement7").innerHTML = checkboxfieldset;
...
/*if (document.getElementById('formElement8')){
	...
	if (document.getElementById('fe110822')){
		var div81a = document.getElementById('fe110822');
		var div81b = div81a.getElementsByTagName('div');
		console.log("div81b  === "+div81b.length);
		div81b[0].setAttribute("style", "width: 80px !important;display: table-cell;vertical-align: top; padding-top:10px;");
		div81b[1].setAttribute("style", "width: 80px !important;display: table-cell;vertical-align: top; padding-top:10px;");
...
if (document.getElementById('formElement8')){
	...
	var checkboxfields = document.getElementById("fe110822").innerHTML;
...
divGDPRtext.innerHTML = "<div style='color: #000;width: 90%;margin-left: 25px;margin-bottom: 30px;margin-top: 15px;font-size:13px;line-height:20px;'>Mit dem Absenden willige ich in die Verarbeitung meiner angegebenen Daten zum Zweck der angegeben Aktion ein. Weitere Informationen finden Sie in unserer <a href='https://www.avaya.com/de/privacy/website/' target='_blank' style='color:#CC0000;font-size:13px;line-height:20px;'>Datenschutzerklärung</a>. Sie können Sie Ihre E-Mail-Einstellungen <a href='https://news.avaya.com/global-subscription' target='_blank' style='color:#CC0000;font-size:13px;line-height:20px;'>hier</a> verwalten.</div>";
...
fe110814.add(Validate.Presence, {failureMessage:"Mussfeld"});
fe110815.add(Validate.Presence, {failureMessage:"Mussfeld"});
fe110816.add(Validate.Presence, {failureMessage:"Mussfeld"});
fe110816.add(Validate.Format, {pattern: /^([^@\s]+)@((?:[-a-z0-9]+\.)+[a-z]{2,})$/i, failureMessage: "Es wird eine gültige E-Mail Adresse benötigt"});
fe110817.add(Validate.Presence, {failureMessage:"Mussfeld"});
fe110818.add(Validate.Presence, {failureMessage:"Mussfeld"});
fe110819.add(Validate.Presence, {failureMessage:"Mussfeld"});
fe110820.add(Validate.Presence, {failureMessage:"Mussfeld"});
var dom8 = document.querySelectorAll('#form3680 [name="AreYouCurrentAvayaCustomer"]');
var fe110924 = new LiveValidation(dom8, {validMessage: "", onlyOnBlur: false, wait: 300, insertAfterWhatNode :this.getParentElement(dom8), isGroup: true}, true);
fe110924.add(Validate.Presence, {failureMessage:"Mussfeld"});
...
var div14a = document.getElementById('fe110850');
...
/*function OnLoadCreateFields(){
	...
	AreYouCurrentAvayaCustomerValid.add( Validate.Presence, { failureMessage: "Mussfeld" } );
	...
	UsingCCSolutionValid.add( Validate.Presence, { failureMessage: "Mussfeld" } );
...
if (getUrlVar("CTA") != ""){
	...
	if (pageURL.includes("/produkte/ccaas/public/")){
		document.getElementsByName("cta_code")[0].value = "WEB-GET-EN-SMB-CC";
		document.getElementsByName("tactic_code")[0].value = "WEB-GET-EN-SMB-CC-DE";
	}else if (pageURL.includes("/produkte/ccaas/public-pricing/")){
		document.getElementsByName("cta_code")[0].value = "WEB-GET-EN-SMB-CC";
		document.getElementsByName("tactic_code")[0].value = "WEB-GET-EN-SMB-CC-PRC-DE";
	} else if (pageURL.includes("/produkte/ccaas/public-features/")){
		document.getElementsByName("cta_code")[0].value = "WEB-GET-EN-SMB-CC";
		document.getElementsByName("tactic_code")[0].value = "WEB-GET-EN-SMB-CC-FEA-DE";
	} 
}
if (getUrlVar("TAC") != ""){
...
(function() {
	...
	if (window.jQuery) {
		...
		$('#formElement8').parents().eq(2).css({'float' : 'left'});
	}
	if(checktimer >= 5){ 
