...
//document.getElementById("formElement0").style.display = "none";//not to display First Name field container
document.getElementById("formElement3").style.display = "none";//not to display Business Phone field container
document.getElementById('formElement4').style.display = "none";//not to display Country field container
document.getElementById('formElement8').style.display = "none";//NameYourCurrentCCSolution
document.getElementById('formElement11').style.display = "none";//address3
document.getElementById("formElement8").style.marginBottom = "20px";
...
BussPhone.setAttribute("id","formElement3a");
BussPhone.setAttribute("class","elq-field-style form-element-layout row");
BussPhone.innerHTML = "<label id='BussPhone' style=float:left;clear:both;'>会社電話番号</label><br><table cellpadding='0' cellspacing='0' width='100%' border='0'><tr><td width='13%'><input id='PhCode' name='PhCode' type='text' value='+81'></td><td width='87%' style='padding-left:10px'><div style='width:100%'><input id='PhNumber' name='PhNumber' type='text' value='' width='100%' onblur='javascript:stripltpspaces(this)'></div></td></tr><tr><td width='100%' colspan='2' style='font-size:10px;padding-top: 5px;font-family:\"ＭＳ Ｐゴシック\",\"MS PGothic\",Arial,sans-serif;'>電話番号を市外局番からハイフン無しで入力願います。</td></tr></table>";
//exInp1.appendChild(newP);
var z1 = document.getElementById("formElement3"); 
//insertBefore(newP, z);
...
document.getElementById('fe122930').value = tp11+" "+tp12;
}
...
//document.getElementById("formElement3a").style.marginBottom = "15px";
...
/*** GDPR Optin / Optout Code **************/
			...
			function checkOptin(){
				...
				if(checkbox1.checked == true){
					...
					document.getElementsByName("emailOptinSource1")[0].value = "WEB-GET-JP-SMB-CC";
...
vUsingCCSolutionYes.onclick = function(){
	...
	//if (document.getElementsByName('UsingCCSolution')[0].value == "Yes"){
		document.getElementById('formElement8').style.display = "block";
		var fe122936 = new LiveValidation( "fe122936", { validMessage: "", onlyOnBlur: true });
		fe122936.enable();
		fe122936.add( Validate.Presence, { failureMessage: "入力必須項目です" } );	
	//} else {		
		//var field9 = new LiveValidation( "field9", { validMessage: "", onlyOnBlur: true });
		//field9.disable();
		//document.getElementById('formElement8').style.display = "none";
	//}	
...
document.getElementById("fe122955").type = "hidden"; //EmailOptin is text field to hide this field
...
if (document.getElementById('formElement6')){
	var div7 = document.getElementById('formElement6');
	...
	if (document.getElementById('fe122934')){
		var div71a = document.getElementById('fe122934');
...
if (document.getElementById('formElement7')){
	var div8 = document.getElementById('formElement7');
	...
	if (document.getElementById('fe122935')){
		var div81a = document.getElementById('fe122935');
...
divGDPRtext.innerHTML = "<div style='color: #000;width: 90%;margin-left: 25px;margin-bottom: 30px;margin-top: 15px;font-size:13px;line-height:20px;'>このフォームを送ることでご自分の個人情報をアバイアのデーターベースにて上記の目的で 使用されることにご同意いただいたことになります。アバイアのプライバシーステイトメントは<a href='https://www.avaya.com/jp/privacy/website/' target='_blank' style='color:#CC0000;font-size:13px;line-height:20px; text-decoration:underline;'>こちら</a>でご確認いただけます。受信解除やメールの設定は<a href='https://news.avaya.com/global-subscription' target='_blank' style='color:#CC0000;font-size:13px;line-height:20px; text-decoration:underline;'>こちら</a>からいつでも変更可能です。</div>";
document.getElementById('formElement12').style.marginTop = "15px";
...
fe122931.add(Validate.Presence, {failureMessage:"会社名をご記入ください."});
fe122928.add(Validate.Presence, {failureMessage:"お名前をご記入ください."});
fe122929.add(Validate.Presence, {failureMessage:"メールアドレスをご記入ください."});
fe122929.add(Validate.Format, {pattern: /^([^@\s]+)@((?:[-a-z0-9]+\.)+[a-z]{2,})$/i, failureMessage: "メールアドレスをご記入ください."});
var PhNumber = document.querySelector('#form3861 #PhNumber');
...
//fe122873.add(Validate.Presence, {failureMessage:"入力必須項目です"});
fe122933.add(Validate.Presence, {failureMessage:"入力必須項目です"});
...
var div14a = document.getElementById('fe122985');
...
function OnLoadCreateFields(){
	var divcontainer1 = document.getElementById('formElement6');
	var divcontainer1a = document.getElementById('formElement6').children[1].children[0].children[0].children[0].children[1];
	var divcontainer1b = document.getElementById('formElement6').children[1].children[0].children[0].children[0];
	divcontainer1b.setAttribute("style","display: table;");
	var textfield1 = document.createElement('div');
	textfield1.setAttribute("id","formElement6a");
	...
	textfield2.setAttribute("id","formElement7a");
	...
	divcontainer2 = document.getElementById('formElement7');
	insertAfter(textfield2, divcontainer2);
	
	var divcontainer2 = document.getElementById('formElement7');
	var divcontainer2a = document.getElementById('formElement7').children[1].children[0].children[0].children[0].children[1];
	var divcontainer2b = document.getElementById('formElement7').children[1].children[0].children[0].children[0];
	...
	AreYouCurrentAvayaCustomerValid.add( Validate.Presence, { failureMessage: "入力必須項目です" } );
	...
	UsingCCSolutionValid.add( Validate.Presence, { failureMessage: "入力必須項目です" } );
...
if (getUrlVar("CTA") != ""){
	...
	if (pageURL.includes("/products/ccaas/public/")){
		...
		document.getElementsByName("tactic_code")[0].value = "WEB-GET-EN-SMB-CC-JP";
	}else if (pageURL.includes("/products/ccaas/public-pricing/")){
		document.getElementsByName("cta_code")[0].value = "WEB-GET-EN-SMB-CC";
		document.getElementsByName("tactic_code")[0].value = "WEB-GET-EN-SMB-CC-PRC-JP";
	} else if (pageURL.includes("/products/ccaas/public-features/")){
		document.getElementsByName("cta_code")[0].value = "WEB-GET-EN-SMB-CC";
		document.getElementsByName("tactic_code")[0].value = "WEB-GET-EN-SMB-CC-FEA-JP";
	} 
}
if (getUrlVar("TAC") != ""){
...
for(var i = 0; i < imgsrc.length - 15; i ++) {
	...
	if (imgsrc2.length > 0){
		...
		//console.log("imgsrc2 ==  "+imgsrc2.length);
			if (imgsrc[i].querySelector("#formElement4") != null){
...
var form = document.getElementById("form3861");
...
/*GA*/
 ...
 document.getElementById('form3861').addEventListener(
