...
/*** GDPR Optin / Optout Code **************/
			...
			function checkOptin(){
				...
				if(checkbox1.checked == true){
					...
					document.getElementsByName("emailOptinSource1")[0].value = "9USA-SUB-DG-LP-CME-KO";
...
labelatt('fe96858');//hide EmailOptin label
...
document.getElementById("fe96858").type = "hidden";//EmailOptin is text field to hide this field
...
divGDPRtext.innerHTML = "<div style='color: #000;width: 90%;margin-left: 25px;margin-bottom: 30px;margin-top: 15px;font-size:13px;line-height:20px;font-family: HCo Gotham SSm;'>이 양식을 제출함으로써 본인은 위에서 언급한 목적으로 어바이어 데이터베이스에서 정보를 처리하는 데 동의합니다.<br> 자세한 내용은 <a href='https://www.avaya.com/kr/privacy/website/' target='_blank' style='color:#DA291C;text-decoration:underline;font-family: HCo Gotham SSm;'>개인 정보 보호 정책</a>을 참조하거나 언제든지 <a href='https://news.avaya.com/global-subscription' target='_blank' style='color:#DA291C;text-decoration:underline;font-family: HCo Gotham SSm;'>이메일 환경 설정</a>을 관리하십시오.</div>";
...
document.getElementsByName("contentTitle1")[0].value = "Avaya IX 구독에 대해 더 알아보기";//Insert content title here
...
//for Validation
fe96840.add(Validate.Presence, {failureMessage:"이 필드는 필수 항목입니다."});
fe96841.add(Validate.Presence, {failureMessage:"이 필드는 필수 항목입니다."});
fe96842.add(Validate.Presence, {failureMessage:"이 필드는 필수 항목입니다."});
fe96842.add(Validate.Format, {pattern: /^([^@\s]+)@((?:[-a-z0-9]+\.)+[a-z]{2,})$/i, failureMessage: "유효한 이메일 주소가 요구됩니다"});
fe96843.add(Validate.Presence, {failureMessage:"이 필드는 필수 항목입니다."});
fe96844.add(Validate.Presence, {failureMessage:"이 필드는 필수 항목입니다."});
fe96845.add(Validate.Presence, {failureMessage:"이 필드는 필수 항목입니다."});
...
/*GA*/
 ...
 document.getElementById('form3359').addEventListener(
