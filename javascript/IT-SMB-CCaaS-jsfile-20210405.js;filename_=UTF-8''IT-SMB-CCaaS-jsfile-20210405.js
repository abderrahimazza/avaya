...
/*** GDPR Optin / Optout Code **************/
			...
			function checkOptin(){
				...
				if(checkbox1.checked == true){
					...
					document.getElementsByName("emailOptinSource1")[0].value = "WEB-GET-IT-SMB-CC";
...
vUsingCCSolutionYes.onclick = function(){
	...
	//if (document.getElementsByName('UsingCCSolution')[0].value == "Yes"){
		...
		var fe110749 = new LiveValidation( "fe110749", { validMessage: "", onlyOnBlur: true });
		fe110749.enable();
		fe110749.add( Validate.Presence, { failureMessage: "Questo campo è obbligatorio" } );	
	//} else {		
...
document.getElementById("fe110768").type = "hidden"; //EmailOptin is text field to hide this field
...
if (document.getElementById('formElement7')){
	...
	if (document.getElementById('fe110747')){
		var div71a = document.getElementById('fe110747');
...
if (document.getElementById('formElement8')){
	...
	if (document.getElementById('fe110748')){
		var div81a = document.getElementById('fe110748');
...
divGDPRtext.innerHTML = "<div style='color: #000;width: 90%;margin-left: 25px;margin-bottom: 30px;margin-top: 15px;font-size:13px;line-height:20px;'>Con l’inoltro di questo modulo, consento ad Avaya di processare le mie informazioni nel suo database per gli scopi sopra menzionati. Vedi il nostro <a href='https://www.avaya.com/it/privacy/website/' target='_blank' style='color:#CC0000;font-size:13px;line-height:20px;'>Privacy Statement</a> per ulteriori dettagli o gestisci le tue <a href='https://news.avaya.com/global-subscription' target='_blank' style='color:#CC0000;font-size:13px;line-height:20px;'>Preferenze Email</a> in qualsiasi momento.</div>";
...
fe110740.add(Validate.Presence, {failureMessage:"Questo campo è obbligatorio"});
fe110741.add(Validate.Presence, {failureMessage:"Questo campo è obbligatorio"});
fe110742.add(Validate.Presence, {failureMessage:"Questo campo è obbligatorio"});
fe110742.add(Validate.Format, {pattern: /^([^@\s]+)@((?:[-a-z0-9]+\.)+[a-z]{2,})$/i, failureMessage: "È necessario un indirizzo email valido"});
fe110743.add(Validate.Presence, {failureMessage:"Questo campo è obbligatorio"});
fe110744.add(Validate.Presence, {failureMessage:"Questo campo è obbligatorio"});
fe110745.add(Validate.Presence, {failureMessage:"Questo campo è obbligatorio"});
fe110746.add(Validate.Presence, {failureMessage:"Questo campo è obbligatorio"});
...
var div14a = document.getElementById('fe110776');
...
function OnLoadCreateFields(){
	...
	UsingCCSolutionValid.add( Validate.Presence, { failureMessage: "Questo campo è obbligatorio" } );
...
if (getUrlVar("CTA") != ""){
	...
	if (pageURL.includes("/prodotti/ccaas/public/")){
		document.getElementsByName("cta_code")[0].value = "WEB-GET-EN-SMB-CC";
		document.getElementsByName("tactic_code")[0].value = "WEB-GET-EN-SMB-CC-IT";
	}else if (pageURL.includes("/prodotti/ccaas/public-pricing/")){
		document.getElementsByName("cta_code")[0].value = "WEB-GET-EN-SMB-CC";
		document.getElementsByName("tactic_code")[0].value = "WEB-GET-EN-SMB-CC-PRC-IT";
	} else if (pageURL.includes("/prodotti/ccaas/public-features/")){
		document.getElementsByName("cta_code")[0].value = "WEB-GET-EN-SMB-CC";
		document.getElementsByName("tactic_code")[0].value = "WEB-GET-EN-SMB-CC-FEA-IT";
	} 
}
if (getUrlVar("TAC") != ""){
