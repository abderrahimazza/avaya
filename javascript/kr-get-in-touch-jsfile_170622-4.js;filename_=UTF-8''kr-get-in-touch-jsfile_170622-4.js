...
document.getElementById('fe85948').value = tp11+" "+tp12;
}
...
labelatt('fe85970');//hide EmailOptin label
...
document.getElementById("fe85970").type = "hidden"; //EmailOptin is text field to hide this field
...
pInstruct.innerHTML = "<font style='color: #313133;font-weight: 500;font-size:12px'><span style='color: #da291c; font-size: 12px;'>*</span> 다음은 모두 필수 입력 사항입니다.</font>";
...
label.appendChild(document.createTextNode("예, 또한 어바이어로부터 마케팅 이메일을 수신하는 데 동의합니다."));
label.setAttribute("for","optin");
var divGDPRtext = document.createElement("div");
divGDPRtext.id="gdprtext";
divGDPRtext.innerHTML = "<div style='color: #313133;width: 95%;margin-left: 5px;margin-bottom: 30px;margin-top: 15px;font-size:12px;line-height:20px;font-family: Noto Sans JP' class='formStatement'>이 양식을 제출함으로써 본인은 위에서 언급한 목적으로 어바이어 데이터베이스에서 정보를 처리하는 데 동의합니다. <br>자세한 내용은 <a href='https://www.avaya.com/kr/privacy/website/' class='formStatLink' target='_blank' style='text-decoration:underline;font-size: 12px;font-family: Noto Sans JP; border-bottom: 0px solid;color:#1B77AF;'>개인 정보 보호 정책</a>을 참조하거나 언제든지 <a href='https://news.avaya.com/global-subscription' class='formStatLink' target='_blank' style='text-decoration:underline;font-size: 12px;font-family: Noto Sans JP; border-bottom: 0px solid;color: #1B77AF;'>이메일 환경 설정</a>을 관리하십시오.</div>";
...
document.getElementById("optin").setAttribute("aria-label", "예, 또한 어바이어로부터 마케팅 이메일을 수신하는 데 동의합니다.");
document.getElementById("PhNumber").setAttribute("aria-label", "직통번");
document.getElementById("PhCode").setAttribute("aria-label", "지역번");
...
var SessTAC = getCookie('tac');
        if (SessCTA != null) {
            ...
            document.getElementsByName("cta_code")[0].value = "WEB-GET-KR";
        }
        if (SessTAC != null) {
            console.log(SessTAC);
            ...
            document.getElementsByName("tactic_code")[0].value = "WEB-GET-KR";
        }*/
...
//for Validation
fe85944.add(Validate.Presence, {failureMessage:"이 필드는 필수 항목입니다."});
fe85945.add(Validate.Presence, {failureMessage:"이 필드는 필수 항목입니다."});
fe85946.add(Validate.Presence, {failureMessage:"이 필드는 필수 항목입니다."});
fe85947.add(Validate.Presence, {failureMessage:"이 필드는 필수 항목입니다."});
fe85947.add(Validate.Format, {pattern: /^([^@\s]+)@((?:[-a-z0-9]+\.)+[a-z]{2,})$/i, failureMessage: "유효한 이메일 주소가 요구됩니다"});

var PhNumber = document.querySelector('#form3032 #PhNumber');
...
fe85949.add(Validate.Presence, {failureMessage:"이 필드는 필수 항목입니다."});
fe85950.add(Validate.Presence, {failureMessage:"이 필드는 필수 항목입니다."});

subonclick= document.getElementById("fe85976");
...
/*GA*/
 ...
 document.getElementById('form3032').addEventListener(
 ...
 document.getElementById('GACLIENTID').value = clientId;
			...
			function checkOptin(){
				...
				if(checkbox1.checked == true){
					...
					document.getElementsByName("emailOptinSource1")[0].value = "WEB-GET-KR";
